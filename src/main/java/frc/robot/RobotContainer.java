...
public class RobotContainer {
    ...
    private void configureBindings() {
        ...
        drivetrain.setDefaultCommand(
                ...
                drivetrain.applyRequest(() -> drive
                        .withVelocityX(
                                -joystick.getLeftY() * MaxSpeed * (joystick.leftTrigger().getAsBoolean() ? .2 : 1)) // Drive
                                                                                                                    // forward
                                                                                                                    // with
                        // negative Y
                        // (forward)
                        .withVelocityY(
                                -joystick.getLeftX() * MaxSpeed * (joystick.leftTrigger().getAsBoolean() ? .2 : 1)) // Drive
                                                                                                                    // left
                                                                                                                    // with
                                                                                                                    // negative
                                                                                                                    // X
                        ...
                        .withRotationalRate(-joystick.getRightX() * MaxAngularRate
                                * (joystick.leftTrigger().getAsBoolean() ? .2 : 1)) // Drive counterclockwise with
                // negative X (left)
                ));
    ...
    public void configureButtons() {
        ...
        joystick.rightTrigger().and(() -> curMode == Mode.CORALMODE)
                .whileTrue(new CoralOuttakeCommand(m_IntakeSubsystem, m_ElevatorSubsystem, m_LedSubsystem));
        ...
        joystick.leftBumper().whileTrue(new ChangeElevatorCommand(m_IntakeSubsystem, m_ElevatorSubsystem, joystick));
        joystick.povCenter().onFalse(new SwitchClimbCommand(m_ClimbSubsystem));
        joystick.x().onFalse(new ThirdAutoAlign(drivetrain, Optional.of(LeftOrRight.LEFT), joystick, m_LedSubsystem));
        joystick.a().onFalse(new ThirdAutoAlign(drivetrain, Optional.of(LeftOrRight.RIGHT), joystick, m_LedSubsystem));
    ...
    public void configureNamedCommands() {
        NamedCommands.registerCommand("Score L2",
                m_ElevatorSubsystem.setCoralHeightCommand(2)
                        ...
                        .andThen(new CoralOuttakeCommand(m_IntakeSubsystem, m_ElevatorSubsystem, m_LedSubsystem)));

        NamedCommands.registerCommand("L3 height", m_ElevatorSubsystem.setCoralHeightCommand(3)
                .alongWith(m_IntakeSubsystem.setPivotCommand(IntakeConstants.kL3ScoringAngle)));
        NamedCommands.registerCommand("L2 height",
                m_ElevatorSubsystem.setCoralHeightCommand(2).alongWith(m_IntakeSubsystem.setPivotCommand()));
        ...
        NamedCommands.registerCommand("Coral Outtake",
                new CoralOuttakeCommand(m_IntakeSubsystem, m_ElevatorSubsystem, m_LedSubsystem).withTimeout(.9));
        NamedCommands.registerCommand("Coral Intake",
                new AutoCoralIntakeCommand(m_ElevatorSubsystem, m_IntakeSubsystem).withTimeout(1.6));
        ...
        NamedCommands.registerCommand("Auto Align Right",
                new ThirdAutoAlign(drivetrain, Optional.of(LeftOrRight.RIGHT), joystick, m_LedSubsystem)
                        .withTimeout(1.5));
        // NamedCommands.registerCommand("Descore Algae Height",
        // m_ElevatorSubsystem.setDescoreHeightCommand().alongWith(m_IntakeSubsystem.setPivotCommand(IntakeConstants.kAlgaeHighIntakeAngle)));
        // NamedCommands.registerCommand("Descore Algae",
        // m_IntakeSubsystem.algeaOutakeCommand().withTimeout(1.2));
